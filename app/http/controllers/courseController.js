...
const Category = require('app/models/user');
const Payment = require('app/models/user');
...
class courseController extends controller {
    ...
    //         return res.render('home/notlogin', { course });
    //     return next();
    ...
    async payment(req, res, next) {
        try {
            ...
            // this.isMongoId(req.params.Course);
            // let episode = await Course.findOneAndUpdate(req.params.course);
            ...
            request(options)
                .then(async data => {
                    ...
                    await payment.save();
                    const mmd = await Course.findOneAndUpdate({ $inc: { tedad: -1 } })
